...
function normalizeImageOptions(options = {}, defaultSmall = true) {
    ...
    const normalizedSize = Array.isArray(options.size) && options.size.length === 2 ? options.size : defaultSize;
    const normalizedMargin = Number.isFinite(options.margin) ? options.margin : defaultMargin;
    const normalizedSmall = options.small === undefined ? Boolean(defaultSmall) : options.small !== false;
...
class StyleObj extends BaseObject {
    ...
    constructor(options = {}) {
        super();
        const { name, type, small = true, default: isDefault = false, conditions = [], flatStyle = {} } = options;
